...
# 确保这些自定义模块和路径函数在您的环境中可用
from path import get_valid_write_path, get_valid_read_path, get_write_directory
...
# --- 🎯 针对 Llama/Qwen 风格命名进行修改 (model.layers.{i}.mlp.down_proj) ---

def get_down_proj_disable_names(num_layers: int) -> list:
    """
    根据 Llama/Qwen 风格命名获取 MLP 投影层 (down_proj) 的禁用名称列表。
    """
...
# 由于您的模型结构是 Llama/Qwen 风格，我们不再需要 get_c_proj_disable_names，
# 但为了兼容旧代码调用，我们让它返回 Llama 风格的名称。
def get_c_proj_disable_names(num_layers: int) -> list:
    """
    兼容函数：返回 Llama/Qwen 风格的 MLP 投影层名称。
    """
    return get_down_proj_disable_names(num_layers)
...
def parse_arguments():
    ...
    parser.add_argument('--model_name', type=str, default='llama', # 默认改为 llama 风格
                         validator=StringArgumentValidator(min_length=1, max_length=MAX_KEY_LENGTH, allow_none=True))
    parser.add_argument('--model_type', type=str, default='llama', # 默认改为 llama 风格
                         choices=['llama', 'qwen', 'gpt2', 'other'],
                         help='Specify the type of the model for layer naming.')
...
class Quantifier:
    def __init__(self, model_path_or_name, args,
        ...
        self.model_name = kwargs.get("model_name", 'llama')
    ...
    def create_quant_config(self, num_layers, select_layer_data=None, ):
        ...
        # --- 使用 Llama 风格的 get_down_proj_disable_names ---
        if not disable_names and args.a_bit in [8, 16]:
            if args.disable_threshold > 0:
                # 自动选择逻辑：使用 Llama 风格的 down_proj 名称
                disable_names = get_down_proj_disable_names(num_layers)
            
            # 如果不自动选择，且模型类型是 Llama/Qwen 风格
            elif args.model_type in ['llama', 'qwen', 'mistral']:
                disable_names = get_down_proj_disable_names(num_layers)
            
            else:
                 # 回退到 Llama 风格作为默认 (因为这是最常见的 Transformer 结构)
                disable_names = get_down_proj_disable_names(num_layers) 
...
if __name__ == '__main__':
    ...
    if args.anti_method == 'm3':
        ...
        # --- M6 目标层使用 Llama 风格命名 ---
        # Attention Output Projection Layer: model.layers.{i}.self_attn.o_proj
    ...
    if args.anti_calib_file:
        ...
        if args.model_type in ['llama', 'qwen', 'mistral']: 
